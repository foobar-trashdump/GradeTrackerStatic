...
public class StudentInputHandler {
    ...
    double inputLabPerformance() {
        ...
        IO.println("  --- Lab Performance (5 modules) ---");
        for (int i = 0; i < GradeConstants.NUM_MODULES; i++) {
            moduleScores[i] = inputComponentScore(
                    "    Module " + (i + 1) + " score: ");
        }

        // [TRACE] Static call — no calculator object needed
        return GradeCalculator.computeAverage(moduleScores);
    }
    ...
    // [TRACE] "11191031" → IDVerifier.validateID returns "Valid student ID number."
    ...
    public String inputIdNumber(int studentNumber) {
        ...
        do {
            ...
            result   = IDVerifier.validateID(idNumber);
            IO.println("  " + result);
        } while (!result.startsWith("Valid"));
